...
// Write `data` to `path`, or under `--check` verify the committed copy is what this run produces.
// Drift names the first differing line: both artifacts are far too large for "stale" to be a
// useful thing to hand someone.
artifact_emit :: proc(data: []byte, path: string, check: bool, d: ^Diags) {
    assert(len(data) > 0, "artifact_emit needs encoded bytes")
    ...
    if err != nil {
        diagf(d, Pos{file = path}, "nothing committed to check against; run `make schema` and commit it")
    ...
    diagf(d, Pos{file = path}, "the committed artifact is stale; run `make schema` and commit the result")
