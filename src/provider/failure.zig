...
pub fn classify(err: anyerror) Detail {
    return switch (err) {
        ...
        => .{ .class = .transport, .code = .network, .message = "the engine could not resolve the provider host" },
        ...
        http.Error.RateLimitUnknown => .{ .class = .permanent, .code = .rate_limited, .message = "the provider returned a 429 the engine could not classify" },
