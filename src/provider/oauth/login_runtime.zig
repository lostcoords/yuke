//! One device login the engine owns. It outlives the connection that asked for it.
...
/// The flows this engine can drive. The catalog row names one; an unknown name is not a login.
pub const Flow = enum {
    xai,
    codex,
...
pub const Logins = struct {
    ...
    map: std.AutoHashMapUnmanaged(proto.ids.LoginId, *LoginSlot) = .empty,
    ...
    pub fn get(self: *Logins, id: proto.ids.LoginId) ?*LoginSlot {
    ...
    pub fn remove(self: *Logins, id: proto.ids.LoginId) void {
...
test "one provider holds one login, and a removal frees it" {
    ...
    const id: proto.ids.LoginId = .bytes(@splat(1));
