...
draw();
r = p.list.drawnRect;
const body = p.bodyRows;
draw();
check(p.bodyRows === body, "wrap cache");
check(p.onMouse({ event: "press", button: "wheel_down", col: r.x, row: w.rect.y + 2, count: 1 }) && p.bodyScroll > 1, "body scroll steps scrollLines");
check(p.list.selected() === "No", "body wheel moved action");
p.onKey({ type: "key", code: "page_down", mods: 0 });
check(p.bodyScroll > 1 && p.list.selected() === "No", "body keyboard scroll");
w.opts.width = 60; w.opts.height = p.preferredHeight(60);
draw();
check(p.bodyRows !== body && p.bodyScroll === 0, "resize wrap/scroll");
check(p.bodyLayout(w.inner).height === p.bodyRows.length, "body clipped at normal size");
check(p.list.drawnRect.h === 2, "preferred height action rows");
w.border = "none"; w.opts.height = p.preferredHeight(60);
draw();
check(p.list.drawnRect.x === w.rect.x && p.list.drawnRect.w === w.rect.w && p.list.drawnRect.h === 2, "borderless preferred height");
