...
const parts = {};
const text = {};
const t = new Transcript({ textOf: (id) => text[id] || "", partsOf: (id) => parts[id] || [] });
...
// A projection that cut a text part cannot place what follows it, so the whole text keeps its place and the labels follow.
text.d = "a very long message the projection could not inline whole";
parts.d = [
  { type: "text", id: 0, text: "a very long message the projection", cut: [{ field: "text", next: 34 }] },
  { type: "image", id: 1, source: png },
];
check("cut-keeps-whole-text", body("d").indexOf("could not inline whole") >= 0);
check("cut-appends-the-label", body("d").indexOf("[PNG #1 · 12 KiB]") >= 0);
