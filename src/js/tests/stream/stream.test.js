...
const beforeSource = t.sourceOf("m");
...
const selectedSource = t.selectedText(true);
const state = t.partStates.get("m");
...
const afterSource = t.sourceOf("m");
...
check("suffix-selection-source", t.selectedText(true) === selectedSource);
...
const replacementSource = replacement.sourceOf("replacement");
...
check("replacement-clears-selection", replacement.selection === null && replacement.selectedText(true) === "");
...
const resizeSource = resize.sourceOf("resize");
...
check("resize-selection", resize.selectedText(true) === resizeSource);
...
const evictionSource = eviction.sourceOf("evict-0");
eviction.select(eviction.posAtSource("evict-0", 0), eviction.posAtSource("evict-0", evictionSource.length));
eviction.rows(30, 0, eviction.rowCount(30));
check("eviction-selection", eviction.selectedText(true) === evictionSource);
