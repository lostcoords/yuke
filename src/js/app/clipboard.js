...
// Chromium apps put only `public.png` on the clipboard, so the read uses NSPasteboard and not the legacy `PNGf` class.
// A TIFF-only clipboard converts to PNG, because the blob store takes no TIFF.
const MAC_READ = [
  'ObjC.import("AppKit");',
  "function run(argv) {",
...
// JavaScript reads no environment, so the shell picks its own branch: it alone knows the system and the tools on it.
// `mktemp` names the file, so no other process can win a race for a name this one predicted.
// The path prints before any work, so a failure still leaves the caller a file to remove.
const READ_IMAGE = [
  'd=${TMPDIR:-/tmp}; d=${d%/}',
