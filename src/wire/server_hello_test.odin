...
@(test)
test_server_hello_capabilities_roundtrip :: proc(t: ^testing.T) {
    context.allocator = context.temp_allocator
    defer free_all(context.temp_allocator)
    ...
    server_hello_emit(&e, h)
    out := to_string(&e)

    v := decoder_init(out)
    decoded, derr := server_hello_from_reader(&v)
    testing.expect(t, derr == .None, "decode should succeed")
    testing.expect_value(t, decoded.capabilities, bit_set[Capability]{.Terminal, .Blob_Upload})
}

@(test)
test_server_hello_ignores_unknown_capability :: proc(t: ^testing.T) {
    context.allocator = context.temp_allocator
    defer free_all(context.temp_allocator)
...
@(test)
test_server_hello_rejects_non_string_capability :: proc(t: ^testing.T) {
    context.allocator = context.temp_allocator
    defer free_all(context.temp_allocator)
    ...
    _, derr := server_hello_from_reader(&v)
