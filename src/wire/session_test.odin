...
test_session_origin_roundtrip :: proc(t: ^testing.T) {
    ...
    input := `{"type":"child","parent_id":"0123456789abcdef"}`
...
test_session_accepts_null_attribution_for_child :: proc(t: ^testing.T) {
    ...
    input := `{"id":"0123456789abcdef","workspace_id":"aaaaaaaaaaaaaaaa","profile":"default","model":"openai/gpt","reasoning":"high","config_rev":1,"permission":"normal","max_rounds":null,"title":"child","message_count":0,"updated_at_ms":1,"created_by":null,"origin":{"type":"child","parent_id":"1111111111111111"}}`
    v := decoder_init(input, context.temp_allocator)
...
test_activity_state_running_roundtrip :: proc(t: ^testing.T) {
    ...
    input := `{"type":"running","run_id":1,"config":{"config_rev":2,"model":"openai/gpt-5.5","reasoning":"high"},"started_at_ms":1720000000000}`
    v := decoder_init(input, context.temp_allocator)
    ...
    testing.expect_value(t, u64(running.config.config_rev), u64(2))
    testing.expect_value(t, running.config.model, "openai/gpt-5.5")
    testing.expect_value(t, running.config.reasoning, "high")
...
test_activity_state_building_roundtrip :: proc(t: ^testing.T) {
    ...
    input := `{"type":"building","run_id":1,"config":{"config_rev":2,"model":"openai/gpt-5.5","reasoning":"high"},"started_at_ms":1720000000000}`
    v := decoder_init(input, context.temp_allocator)
...
@(test)
test_system_prompt_patch_roundtrip :: proc(t: ^testing.T) {
    context.allocator = context.temp_allocator
    defer free_all(context.temp_allocator)

    input := `{"type":"set","value":"be terse"}`
    v := decoder_init(input, context.temp_allocator)
    patch, derr := system_prompt_patch_from_reader(&v)
    testing.expect(t, derr == .None, "decode should succeed")
    set, ok := patch.(System_Prompt_Patch_Set)
    testing.expect(t, ok, "should be a set patch")
    testing.expect_value(t, set.value, "be terse")

    e: Emitter
    emitter_init(&e)
    defer emitter_destroy(&e)
    system_prompt_patch_emit(&e, patch)
    testing.expect_value(t, to_string(&e), input)

    // The clear arm rejects a sibling value field.
    bad := decoder_init(`{"type":"clear","value":"x"}`, context.temp_allocator)
    _, cderr := system_prompt_patch_from_reader(&bad)
    testing.expect(t, cderr == .Mismatched_Payload, "value under clear must be rejected")
}
...
test_session_rejects_missing_created_by :: proc(t: ^testing.T) {
    ...
    input := `{"id":"0123456789abcdef","workspace_id":"aaaaaaaaaaaaaaaa","profile":"default","model":"openai/gpt","reasoning":"high","config_rev":1,"permission":"normal","max_rounds":null,"title":"child","message_count":0,"updated_at_ms":1,"origin":{"type":"child","parent_id":"1111111111111111"}}`
    context.allocator = context.temp_allocator
    defer free_all(context.temp_allocator)
    v := decoder_init(input, context.temp_allocator)
