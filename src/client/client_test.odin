...
_rec_on_broadcast :: proc(c: ^Client, bc: wire.Notification) {
    s := (^Sink)(c.user_data)
    s.broadcasts += 1
    s.last_bc_name = bc.method

    if s.want_clone {
        s.cloned = wire.notification_clone(bc, s.clone_alloc)
        s.has_clone = true
    }
}
...
test_two_in_flight_requests_reach_their_own_completion :: proc(t: ^testing.T) {
    ...
    list_raw := `{"jsonrpc":"2.0","id":2,"result":{"revision":0,"items":[],"next_cursor":null,"total":0}}`
    ...
    refresh_raw := `{"jsonrpc":"2.0","id":1,"result":{"catalog_rev":"2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824","health":{"skipped":[],"load_error":null}}}`
...
test_duplicate_response_does_not_refire_completion :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","id":3,"result":{"type":"queued","input_id":8}}`
...
test_handle_text_routes_error_response :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","id":4,"error":{"code":-31015,"message":"busy"}}`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.None)
    testing.expect_value(t, sink.responses, 1)
...
test_handle_text_routes_known_broadcast :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","method":"notice","params":{"level":"warn","source":"provider","message":"rate limited"}}`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.None)
...
test_handle_text_unknown_broadcast_not_materialized :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","method":"totally.unknown","params":{"whatever":{"deep":[1,2,3]}}}`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.None)
...
test_handle_text_unknown_response_id :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","id":99,"result":{"type":"queued","input_id":8}}`
    err := client_handle_text(&c, transmute([]byte)raw)
...
test_handle_text_error_unknown_id_accepted_and_validated :: proc(t: ^testing.T) {
    ...
    raw := `{"jsonrpc":"2.0","id":77,"error":{"code":-31015,"message":"busy"}}`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.None)
    testing.expect_value(t, sink.responses, 0)
    testing.expect_value(t, sink.errors, 0)

    // Still fully decoded and validated: an unrecognized error code on an uncorrelated
    // id is rejected exactly as it would be on a correlated one.
    bad := `{"jsonrpc":"2.0","id":77,"error":{"code":-31999,"message":"busy"}}`
...
test_handle_text_trailing_bytes_rejected :: proc(t: ^testing.T) {
    ...
    // A complete notification followed by a trailing token: rejected by `dec_finish`
    // before `on_broadcast` fires.
    raw := `{"jsonrpc":"2.0","method":"notice","params":{"level":"info","source":"x","message":"y"}} 5`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.Decode_Failed)
    testing.expect_value(t, sink.broadcasts, 0)
}

// A duplicate `initialize` response is just a response whose id the handshake already
// consumed, so it fails correlation like any other unknown id.
@(test)
test_handle_text_stray_initialize_response_is_unknown :: proc(t: ^testing.T) {
    sink: Sink
    c: Client
    _init_client(&c, &sink)
    defer _teardown(&c)
...
@(test)
test_handle_hello_malformed_fails_decode :: proc(t: ^testing.T) {
    sink: Sink
    c: Client
    _init_client(&c, &sink)
    c.state = .Awaiting_Initialize
    ...
    testing.expect_value(t, err, Protocol_Error.Decode_Failed)
    testing.expect_value(t, sink.errors, 0)
    testing.expect_value(t, c.state, Protocol_State.Awaiting_Initialize)
}

// A daemon that refuses `initialize` answers with an error response; the handshake
// fails on the outcome rather than on decoding.
@(test)
test_handle_hello_error_response_is_bad_initialize :: proc(t: ^testing.T) {
    sink: Sink
    c: Client
    _init_client(&c, &sink)
    c.state = .Awaiting_Initialize
    ...
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.Bad_Initialize)
    testing.expect_value(t, c.state, Protocol_State.Awaiting_Initialize)
}

// Nothing is pushed before the handshake completes.
@(test)
test_notification_before_ready_is_refused :: proc(t: ^testing.T) {
    sink: Sink
    c: Client
    _init_client(&c, &sink)
    c.state = .Awaiting_Initialize
    ...
    raw := `{"jsonrpc":"2.0","method":"notice","params":{"level":"warn","source":"p","message":"m"}}`
    err := client_handle_text(&c, transmute([]byte)raw)
    testing.expect_value(t, err, Protocol_Error.Bad_Initialize)
    testing.expect_value(t, sink.broadcasts, 0)
}
...
test_broadcast_clone_survives_source_and_scratch :: proc(t: ^testing.T) {
    ...
    src := `{"jsonrpc":"2.0","method":"notice","params":{"level":"warn","source":"provider","message":"rate limited"}}`
    buf := make([]byte, len(src), context.allocator)
    copy(buf, src)
    defer delete(buf, context.allocator)
    ...
    notice, ok := sink.cloned.params.(wire.Notice)
    testing.expect(t, ok, "cloned payload is a notice")
    testing.expect_value(t, sink.cloned.method, wire.Broadcast_Name.Notice)
