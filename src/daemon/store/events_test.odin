...
test_append_recovers_across_restart :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, event_append(s, alpha, 1, .Run_Started, `{"seq":1}`, {run_id = 1}), Error.None)
    testing.expect_value(t, event_append(s, beta, 1, .Run_Started, `{"seq":1}`, {run_id = 1}), Error.None)
    testing.expect_value(t, event_append(s, alpha, 2, .Message_Committed, `{"seq":2}`, {message_id = 7}), Error.None)
    testing.expect_value(t, event_append(s, beta, 2, .Config_Changed, `{"seq":2}`, {config_rev = 3}), Error.None)
    testing.expect_value(t, event_append(s, alpha, 3, .Run_Done, `{"seq":3}`), Error.None)

    testing.expect_value(t, bump_ids(s, alpha, {input_id = 5}), Error.None)

    before_alpha, ba_err := high_water(s, alpha)
    testing.expect_value(t, ba_err, Error.None)
    before_beta, bb_err := high_water(s, beta)
    testing.expect_value(t, bb_err, Error.None)
    ...
    testing.expect_value(t, aa_err, Error.None)
    ...
    testing.expect_value(t, ab_err, Error.None)
    ...
    testing.expect_value(t, event_append(reopened, alpha, 3, .Run_Done, `{"replay":true}`), Error.Constraint)
    testing.expect_value(t, event_append(reopened, alpha, 5, .Run_Done, `{"gap":true}`), Error.Seq_Conflict)
    testing.expect_value(t, event_append(reopened, alpha, 4, .Run_Done, `{"seq":4}`), Error.None)

    // (c) the tail reads back whole and in order.
    events, events_err := events_after(reopened, alpha, 0, 16)
    testing.expect_value(t, events_err, Error.None)
...
test_deleted_tail_does_not_reclaim_seq :: proc(t: ^testing.T) {
    ...
    for seq in wire.Seq(1) ..= 4 {
        testing.expect_value(t, event_append(s, session, seq, .Run_Done, `{"n":0}`), Error.None)
    }
    ...
    testing.expect_value(t, hw_err, Error.None)
    testing.expect_value(t, hw.seq, wire.Seq(4))

    // The deleted numbers are spent: only 5 continues the log.
    testing.expect_value(t, event_append(reopened, session, 3, .Run_Done, `{"n":3}`), Error.Seq_Conflict)
    testing.expect_value(t, event_append(reopened, session, 4, .Run_Done, `{"n":4}`), Error.Seq_Conflict)
    testing.expect_value(t, event_append(reopened, session, 5, .Run_Done, `{"n":5}`), Error.None)
...
test_failed_append_leaves_the_mark_untouched :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, event_append(s, session, 1, .Run_Started, `{"n":1}`, {run_id = 9}), Error.None)

    // Same seq, so the primary key rejects the row and the transaction unwinds
    // both writes.
    testing.expect_value(t, event_append(s, session, 1, .Run_Done, `{"n":1}`, {run_id = 99}), Error.Constraint)
    ...
    // A gap rolls back the event row the guard never sanctioned.
    testing.expect_value(t, event_append(s, session, 3, .Run_Done, `{"n":3}`, {run_id = 42}), Error.Seq_Conflict)
...
test_tail_read_honors_from_seq_and_limit :: proc(t: ^testing.T) {
    ...
    for seq in wire.Seq(1) ..= 6 {
        ...
        testing.expect_value(t, event_append(s, session, seq, .Run_Done, payload), Error.None)
    }

    page, page_err := events_after(s, session, 2, 3)
    testing.expect_value(t, page_err, Error.None)
    ...
    testing.expect_value(t, tail_err, Error.None)
    ...
    testing.expect_value(t, past_err, Error.None)
...
@(test)
test_id_marks_only_rise :: proc(t: ^testing.T) {
    path := testsupport.sqlite_db_path(t, "marks")
    defer testsupport.sqlite_db_remove(path)

    session := test_session(0x17)

    s, err := open(path)
    testing.expect_value(t, err, Error.None)
    defer close(s)

    // A bump on a session with no rows creates its meta row.
    testing.expect_value(t, bump_ids(s, session, {message_id = 4, input_id = 2}), Error.None)

    hw, hw_err := high_water(s, session)
    testing.expect_value(t, hw_err, Error.None)
    testing.expect_value(t, hw, High_Water{seq = 0, ids = {message_id = 4, input_id = 2}})

    // A stale mark never rewinds a live one; the untouched families stay put.
    testing.expect_value(t, bump_ids(s, session, {message_id = 1, run_id = 3}), Error.None)
    testing.expect_value(t, event_append(s, session, 1, .Run_Started, `{"n":1}`, {run_id = 2}), Error.None)

    after, after_err := high_water(s, session)
    testing.expect_value(t, after_err, Error.None)
    testing.expect_value(t, after, High_Water{seq = 1, ids = {message_id = 4, run_id = 3, input_id = 2}})
}
...
test_unknown_stored_name_fails_the_read :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, event_append(s, session, 1, .Run_Started, `{"n":1}`), Error.None)
    ...
    events, events_err := events_after(s, session, 0, 16)
    testing.expect_value(t, events_err, Error.Read_Failed)
    ...
    testing.expect_value(t, prefix_err, Error.None)
...
test_persisted_values_are_validated_without_asserting :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, event_append(s, session, 1, .Run_Started, `{}`), Error.None)
    ...
    testing.expect_value(t, high_err, Error.Read_Failed)
    ...
    testing.expect_value(t, rows_err, Error.Read_Failed)
