//! The RPC suite. It drives the production seam in `rpc.zig` over a real daemon state.
...
fn call(fixture: *TestState, frame: []const u8, buffer: []u8) ![]const u8 {
    ...
    _ = try rpc.handleRequest(&fixture.state, fixture.conn, &out, frame);
    return out.buffered();
}
...
test "session.create defaults the workspace to home and stacks sessions" {
    ...
    ;
    _ = try rpc.handleRequest(&fixture.state, fixture.conn, &out1, empty_params);
    ...
    _ = try rpc.handleRequest(&fixture.state, fixture.conn, &out2, empty_params);
