...
the config revisions referenced by its page plus the current revision.

Provider auth keeps its store, curl client, callback listener, timers, and active work
in `Provider_Auth`. Codex and xAI support browser and device login. Login, refresh, and
credential mutation are single-flight; every login has a deadline. Credential files
are replaced atomically, and a terminal refresh rejection removes the stale credential
