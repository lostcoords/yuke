...
const max_id_bytes = proto.ids.max_selector_part_bytes;
...
pub fn decode(arena: std.mem.Allocator, body: []const u8) Error!Document {
    ...
    if (!proto.ids.CatalogRev.validText(doc.catalog_rev)) return error.InvalidDocument;
    if (doc.providers.len > max_providers) return error.InvalidDocument;

    var models: usize = 0;
    for (doc.providers, 0..) |p, i| {
        if (!bounded(p.id, max_id_bytes) or !proto.ids.isSelectorPart(p.id)) return error.InvalidDocument;
        ...
        for (p.models, 0..) |m, j| {
            ...
            if (!proto.ids.isSelectorTail(m.id)) return error.InvalidDocument;
