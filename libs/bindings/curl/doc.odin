...
Two layers, one package:
  ...
  - `curl.odin`: `Client` and `Turn`. A `Client` owns the multi handle and one
    re-armed `nbio.timeout` that calls `curl_multi_perform` and drains
    `curl_multi_info_read`. The timer is armed when the first turn starts and
    disarmed when the last one ends, so an idle client costs nothing.
...
and are valid for the call only; `Result.message` borrows the turn's error
...
does not copy, so the bytes must stay alive until `On_Done` fires or the turn is
canceled. URL and header lines are copied by libcurl during `turn_start`.

Addresses are pinned: libcurl and the pump timer hold the `Client` and each live
`Turn` by address, so neither may be moved, copied, or reallocated once started.
A caller embedding a `Turn` in its own struct must keep that struct in place.

Cancellation has exactly two forms. `On_Body` returning false aborts from inside
a callback; the turn still completes through `On_Done` with `.Write_Error`.
`turn_cancel` ends a turn from outside every callback: the handle is removed and
destroyed synchronously and no callback ever fires again, mirroring
`nbio.remove`'s final-and-silent contract. Calling `turn_cancel` (or
`turn_start`) from inside a curl callback is a programmer error — libcurl
